# PLAN-DRAFT: Project Init Entrypoint Workflow Integration

Draft status: non-active planning draft
Created at: 2026-04-28
Runtime impact: this draft does not create an active plan, does not create `tasks.json`, and does not modify `work/workflow-state.json`.

## Review Findings Before Draft

No direction-level reversal is needed: `project-init` should still connect target project entrypoints to Harness workflow semantics without copying this repository's full `AGENTS.md` into the target project.

Corrections and missing points found during review:
...
- Do not create an active plan package or change workflow state as part of this draft.
...
The implementation should be split into three delivery tasks.
...
Checks to run before converting this draft into an active plan:
- Scope and non-scope still preserve the LLM/script boundary.
...
Findings:
- Draft only. No pass/fail review has been recorded.
